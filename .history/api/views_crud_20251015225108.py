...
def editar_tratamiento(request, id):
    """Vista para editar tratamiento - CON DIAGNÓSTICO VISUAL"""
    try:
        print("🔍 INICIANDO VISTA editar_tratamiento")
        
        # Obtener el tratamiento
        tratamiento = Tratamiento.objects.get(id=id)
        print(f"🔍 Tratamiento encontrado: {tratamiento.id}")
        
        # OBTENER LOS MEDICAMENTOS
        medicamentos = Medicamento.objects.all()
        print(f"🔍 Medicamentos en BD: {medicamentos.count()}")
        
        # Crear medicamentos de prueba si no existen
        if medicamentos.count() == 0:
            print("🔍 No hay medicamentos, creando de prueba...")
            Medicamento.objects.create(
                nombre="Paracetamol", 
                laboratorio="Genfar", 
                stock=100, 
            ...
            medicamentos = Medicamento.objects.all()
            print(f"🔍 Medicamentos creados: {medicamentos.count()}")
        ...
        if request.method == 'POST':
            ...
            try:
                ...
                print(f"  - Medicamento: {medicamento_nombre}")
                print(f"  - Dosis: {dosis}")
        ...
        print(f"  - Tratamiento: {tratamiento.id}")
...
def editar_tratamiento(request, id):
    """Vista para editar tratamiento - CORREGIDA"""
    try:
        tratamiento = Tratamiento.objects.get(id=id)
        consultas = Consulta.objects.all().select_related('paciente', 'medico')
        medicamentos = Medicamento.objects.all()  # ← OBTENER LOS MEDICAMENTOS
            ...
            try:
                ...
                if not all([consulta_id, medicamento_nombre, dosis, duracion]):
                    ...
                    return render(request, 'editar_tratamiento.html', {
                        'tratamiento': tratamiento,
                        'consultas': consultas,
                        'medicamentos': medicamentos  # ← INCLUIR EN ERROR TAMBIÉN
                    })
        ...
        # Pasar medicamentos al contexto
        return render(request, 'editar_tratamiento.html', {
            'tratamiento': tratamiento,
            'consultas': consultas,
            'medicamentos': medicamentos  # ← ESTA LÍNEA ES CLAVE
        })
