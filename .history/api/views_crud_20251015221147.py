...
def editar_tratamiento(request, id):
    """Vista para editar tratamiento - CON DIAGNÓSTICO"""
    try:
        ...
        print("=" * 50)
        print("🔍 DIAGNÓSTICO - EDITAR TRATAMIENTO")
        print(f"📋 Tratamiento ID: {tratamiento.id}")
        print(f"💊 Medicamento actual: '{tratamiento.medicamento}'")
        print(f"📦 Total de medicamentos en BD: {medicamentos.count()}")
        
        # Mostrar todos los medicamentos disponibles
        for i, med in enumerate(medicamentos):
            print(f"  {i+1}. {med.nombre} (Lab: {med.laboratorio})")
        print("=" * 50)
        ...
        if request.method == 'POST':
            try:
                ...
                print(f"  - Medicamento seleccionado: '{medicamento_nombre}'")
                print(f"  - Dosis: '{dosis}'")
        ...
        # Pasar datos al template - ESTO ES LO MÁS IMPORTANTE
        context = {
            'tratamiento': tratamiento,
            'consultas': consultas,
        ...
        print(f"📤 Enviando al template: {len(medicamentos)} medicamentos")
        return render(request, 'editar_tratamiento.html', context)
