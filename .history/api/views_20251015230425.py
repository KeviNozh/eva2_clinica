...
def crear_medico(request):
    especialidades = Especialidad.objects.all()  # O el queryset que uses
    
    if request.method == 'POST':
        # Tu lógica para crear médico
        pass
    else:
        context = {
...
def crear_especialidad(request):
    if request.method == 'POST':
        try:
            ...
            print(f"Descripción: '{request.POST.get('descripcion', 'VACIO')}'")
            print("Todos los datos:", request.POST.dict())
            ...
            print(f"✅ GUARDADO: {especialidad.nombre} - Descripción: '{especialidad.descripcion}'")
            # ⚠️ FIN DEBUG ⚠️
...
# VISTA PARA EDITAR ESPECIALIDAD - AGREGAR ESTO
def editar_especialidad(request, id):
    """Vista para editar especialidad - CORREGIDA"""
    especialidad = get_object_or_404(Especialidad, id=id)
    
    if request.method == 'POST':
        try:
            # CORREGIDO: Actualiza ambos campos
            especialidad.nombre = request.POST['nombre']
            especialidad.descripcion = request.POST.get('descripcion', '')  # ← ESTA LÍNEA ES CLAVE
            especialidad.save()
...
def lista_especialidades(request):
    ...
    print("=" * 50)
    print("DEBUG - ESPECIALIDADES EN BD:")
    for esp in especialidades:
        print(f"ID: {esp.id} | Nombre: {esp.nombre} | Descripción: '{esp.descripcion}'")
    print("=" * 50)
...
def crear_medico(request):
    ...
    if request.method == 'POST':
        try:
            Medico.objects.create(
                ...
                especialidad_id=request.POST['especialidad'],  # Usar especialidad_id
    ...
    # Pasar las especialidades al contexto
...
def editar_medico(request, id):
    ...
    if request.method == 'POST':
        try:
            ...
            medico.especialidad_id = request.POST['especialidad']  # Usar especialidad_id
...
def editar_receta(request, id):
    ...
    if request.method == 'POST':
        try:
            ...
            indicaciones = request.POST.get('instrucciones')
            ...
            if medico_id:
                nuevo_medico = Medico.objects.get(id=medico_id)
                receta.medico = nuevo_medico
    ...
    return render(request, 'crud/editar_receta.html', {
        'receta': receta,
        'medicos': medicos
    })