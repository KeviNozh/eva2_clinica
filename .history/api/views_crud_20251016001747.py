...
from .forms import PacienteForm, MedicoForm, MedicamentoForm, EspecialidadForm, SalaForm, ConsultaForm, TratamientoForm, RecetaForm, SeguimientoPacienteForm

# ========== VISTAS DE LISTADO (READ) ==========

def crud_pacientes(request):
    """Vista para listar y filtrar pacientes"""
    pacientes = Paciente.objects.all()
    
    # Obtener los valores de los filtros desde la URL (método GET)
    query = request.GET.get('q', '')
    genero = request.GET.get('genero', '')
    tipo_sangre = request.GET.get('tipo_sangre', '')
    activo = request.GET.get('activo', '')
    
    # Aplicar filtro de búsqueda por texto (nombre, apellido, RUT)
    if query:
    ...
    # Aplicar filtro por género si se seleccionó uno
    if genero:
        pacientes = pacientes.filter(genero=genero)
    
    # Aplicar filtro por tipo de sangre si se seleccionó uno
    if tipo_sangre:
        pacientes = pacientes.filter(tipo_sangre=tipo_sangre)
    
    # Aplicar filtro por estado (Activo/Inactivo) si se seleccionó uno
    if activo:
...
def crud_medicos(request):
    """Vista para listar y filtrar médicos"""
...
def crud_consultas(request):
    consultas = Consulta.objects.select_related('paciente', 'medico__user').all()
    return render(request, 'crud_consultas.html', {'consultas': consultas})

def crud_tratamientos(request):
    tratamientos = Tratamiento.objects.select_related('consulta__paciente', 'consulta__medico__user').all()
    return render(request, 'crud_tratamientos.html', {'tratamientos': tratamientos})

def crud_recetas(request):
    recetas = Receta.objects.select_related('consulta__paciente', 'consulta__medico__user', 'medicamento').all()
    return render(request, 'crud_recetas.html', {'recetas': recetas})

def crud_seguimientos(request):
    seguimientos = SeguimientoPaciente.objects.select_related('paciente', 'medico__user').all()
...
def crear_medico(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, '✅ Médico y usuario creados exitosamente!')
...
def crear_sala(request):
    if request.method == 'POST':
        ...
        form = SalaForm()
    especialidades = Especialidad.objects.all()
    return render(request, 'crear_sala.html', {'form': form, 'especialidades': especialidades})
...
def crear_consulta(request):
    if request.method == 'POST':
        ...
        form = ConsultaForm()
    # Pasamos los querysets necesarios para los dropdowns
    context = {
        'form': form,
    ...
    return render(request, 'crear_consulta.html', context)
...
def crear_tratamiento(request):
    ...
    return render(request, 'crear_tratamiento.html', context)
...
def editar_medico(request, id):
    ...
    especialidades = Especialidad.objects.all()
    return render(request, 'editar_medico.html', {'form': form, 'medico': medico, 'especialidades': especialidades})
...
def editar_sala(request, id):
    ...
    especialidades = Especialidad.objects.all()
    return render(request, 'editar_sala.html', {'form': form, 'sala': sala, 'especialidades': especialidades})
...
def eliminar_medico(request, id):
    medico = get_object_or_404(Medico, id=id)
    if request.method == 'POST':
        medico.user.delete()  # Elimina el usuario asociado también
        medico.delete()
