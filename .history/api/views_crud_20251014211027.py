...
def editar_tratamiento(request, id):
    ...
    if request.method == 'POST':
        # Tu lógica actual para guardar los cambios
        pass
    else:
        # Obtener todas las consultas y medicamentos para los dropdowns
        consultas = Consulta.objects.all()
        medicamentos = Medicamento.objects.filter(disponible=True)  # Solo medicamentos disponibles
        
        context = {
            'tratamiento': tratamiento,
            'consultas': consultas,
            'medicamentos': medicamentos  # ← AÑADIR ESTA LÍNEA
        }
...
def editar_tratamiento(request, id):
    ...
    if request.method == 'POST':
        try:
            ...
            tratamiento.fecha_inicio = request.POST['fecha_inicio']
            tratamiento.fecha_fin = request.POST['fecha_fin']
