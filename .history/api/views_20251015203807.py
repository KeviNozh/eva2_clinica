...
def crear_especialidad(request):
    if request.method == 'POST':
        try:
            print("Datos recibidos:", request.POST)  # Para debug
            
            # CREAR la especialidad con descripción
            especialidad = Especialidad.objects.create(
                nombre=request.POST['nombre'],
                descripcion=request.POST.get('descripcion', '')  # ← ESTO ES CLAVE
            )
            
            print(f"Especialidad creada: {especialidad.nombre} - Descripción: '{especialidad.descripcion}'")
