...
def editar_sala(request, id):
    ...
    especialidades = Especialidad.objects.all()  # ← OBTENER ESPECIALIDADES
    ...
    if request.method == 'POST':
        try:
            ...
            # AGREGAR ESTO SI EL MODELO SALA TIENE CAMPO ESPECIALIDAD:
            # sala.especialidad_id = request.POST.get('especialidad')
            sala.save()
    ...
    return render(request, 'editar_sala.html', {
        'sala': sala,
        'especialidades': especialidades  # ← PASAR AL TEMPLATE
    })
