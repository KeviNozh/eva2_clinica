...
class SalaForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Agregar clases CSS a los campos si lo deseas
...
def crear_sala(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('lista_salas')  # Asegúrate de que este nombre coincida con tu URL
    else:
        form = SalaForm()
...
def lista_salas(request):
    salas = Sala.objects.all().order_by('nombre')  # Ordenar por nombre
